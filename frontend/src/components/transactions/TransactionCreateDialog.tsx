// frontend/src/components/transactions/TransactionCreateDialog.tsx
...
export function TransactionCreateDialog({
  ...
  const handleCreate = () => {
    ...
    const data: CreateTransactionData = {
      ...
      cost_center_name: formData.cost_center_name || undefined,
      spend_category_names: categoryNames.length > 0 ? categoryNames : undefined,
  ...
  return (
    <Dialog
      ...
      fullWidth
      TransitionProps={{
        onExited: () => setFormData(getInitialFormData())
      }}
      PaperProps={{
    ...
    >
      ...
      <DialogActions sx={{ p: 3, gap: 1.5, borderTop: '1px solid #e2e8f0' }} >
        ...
        >
          {loading ? <CircularProgress size={20} sx={{ color: '#ffffff' }} /> : 'Add Transaction'}
        </Button>
      </DialogActions>
    </Dialog>
  );
